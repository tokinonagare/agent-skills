...
2. **UI 截图存证 (强制)**：如果提交涉及 UI 变更（样式、组件、布局），必须提交截图。
    - **技术要求**：使用 `agent-browser` 获取截图。
    - **视口规范**：必须使用移动端比例（如 `--device "iPhone 15 Pro"` 或宽度 `375px`）。
    - **内容完整**：严禁提交加载中（Loading）或启动弹窗遮挡的截图。必须通过 `click` 或 `wait` 进入真实业务页面后再截图。
    - **禁止留白**：必须使用 `--selector "#root"` (或等效容器) 进行精准裁剪，彻底消除 Web 端左右两侧的淡蓝色/白色空白边距。
