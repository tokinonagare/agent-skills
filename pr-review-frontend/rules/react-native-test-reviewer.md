...
- ❌ **禁止暴露内部实现**：测试描述不应包含组件内部方法名、私有函数名等实现细节（如 `it('handleRoomAddTime: 接收到事件时...')`、`describe('show 方法')`、`describe('hide 方法')`），应聚焦行为描述（如 `it('接收到加秒事件时，如果 roomId 不匹配则不应更新', () => {})`、`describe('展示普通操作面板时')`、`describe('隐藏面板时')`）
...
- **测试标题、前置条件、触发动作、断言目标必须一致**。如果描述声明了特定业务状态或分支（如 `isAutoFold=true`、`有 check 选项`、`展示普通操作面板`），测试必须通过真实可观察的入口把组件带到该状态，再验证对应行为。
- ❌ 禁止“标题测 A，代码实际测 B”。例如描述写“`isAutoFold=true` 且有 check 时...”，但实际只调用 `show(null)` 并断言其他分支，这属于**错误覆盖**，不能算覆盖该业务逻辑。
- ✅ 优先通过用户交互或公开接口建立状态，例如先 `fireEvent.press` 自动弃牌按钮，再调用公开入口并断言 `onPressCheck` 是否触发。
- 发现“描述与验证逻辑不一致”问题时，置信度应评为 **90-100**，因为这会直接制造虚假的覆盖结论。
...
- **90-100**: 明确违反以上规则（如测试文件位置错误、使用 `toHaveTextContent`、断言 style 属性/颜色值/布局属性、测试组件内部 state、**引用错误的组件实例进行断言**、**测试描述暴露内部实现方法名/覆盖率目标**、**测试标题与实际验证逻辑不一致导致错误覆盖**、**测试中使用真实 timer（setTimeout/sleep/delay/wait）进行固定等待**）
